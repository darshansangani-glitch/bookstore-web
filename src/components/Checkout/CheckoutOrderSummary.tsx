...
export default function CheckoutOrderSummary() {
  ...
  const totalRentPrice = rentBooksData.reduce(
    (total, book) => total + book.rentPrice,
    0,
  );

  const totalBuyPrice = buyBooksData.reduce(
    (total, book) => total + book.buyPrice,
    0,
  );

  const subTotal = totalBuyPrice + totalRentPrice;
